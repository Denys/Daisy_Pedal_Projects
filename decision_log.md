...
## ADR-0006 — Accepted — Keep unverified local EDA imports outside publication

- **Date:** 2026-08-24
- **Lane:** verification/productization
- **Supersedes:** none.
- **Context:** The primary checkout contained 211 untracked entries: generated import history, previews and logs; 48 Altium project/schematic/PCB/library candidates; one KiCad history entry; and a stale local HTML report. The tracked hardware authority uses KiCad formats, no Altium-format source is tracked at the current base, and filenames do not establish provenance or equivalence.
- **Decision:** Preserve the complete local backlog without publishing or deleting it. Apply exact root-anchored ignore rules to the two bounded EDA import workspaces, the bounded KiCad `.history` path, and the stale 2026-07-23 HTML report. Keep the 48 Altium candidates on `HOLD` until a separately authorized provenance and correspondence review establishes whether any file belongs in version control. Ignoring a path is repository-status policy only; it is not an obsolescence, equivalence, or deletion decision.
- **Alternatives rejected:** bulk publication of all 211 entries; blind `git add -A`; deletion based on extension or folder name; treating imported Altium files as authoritative because their names resemble tracked KiCad projects; updating or publishing the stale HTML without a whole-artifact state migration and rendered validation.
- **Evidence:** `origin/main` base `bdb53ce13e83736b6909618adbfda3e237623649`; prior local `main` `613eb4f5014ac0aff8aaeed22952d42a580d11f4`; classification and exact-path inventory in `work_products/automation_runs/daisy_pedal_projects_local_backlog_review_2026-08-24.md`; tracked-file extension inventory; current `.gitignore`; current tracked KiCad hardware files.
- **Validation status:** `PASS` for local preservation, classification totals, and representative ignore-rule matching; `NOT_RUN` for Altium provenance/equivalence, HTML migration/rendered QA, hardware measurements, and on-device validation.
- **Consequences:** normal Git status can remain clean after the ignore policy reaches `main`, while the local files remain physically available. Future hardware-source promotion must explicitly override the matching ignore rule and provide provenance, authoritative-source correspondence, licensing, and tool-open validation. This decision does not alter firmware, DSP, hardware design, submodules, dependencies, or existing pull request #2.
- **Required validation:** before tracking any held Altium candidate, inventory exact files and hashes, establish origin/licensing, compare schematic nets and PCB identity against the current authoritative KiCad sources, open the files with the declared EDA tool/version, and record accepted/rejected correspondence. Before publishing a successor HTML report, perform a complete current-state migration and rendered artifact-quality gate.
